...
#loading the model 
fraud_detection_model = pickle.load(open("C:\\Users\\AMANDEEP KAUR\\Desktop\\CreditCardFraudDetection\\Fraud_Detection.sav", 'rb'))

#creating the side bar
with st.sidebar:
    selected = option_menu("Created by -  Amandeep Kaur",['Credit Card Fraud Detection System'])

if (selected == 'Credit Card Fraud Detection System'):
    
    st.title("Credit Card Fraud Transaction App")
    ...
    if st.button("Detect the Transaction"):
        ...
        if (detection[0]==1):
            fraud_detection = "FRAUD Transaction"
        else:
